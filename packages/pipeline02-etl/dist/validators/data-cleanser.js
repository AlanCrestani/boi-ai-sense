...
export class Pipeline02DataCleanser {
    ...
    cleanRow(mappedRow) {
        ...
        const cleaned = {};
        // Clean data_ref
        const cleanedDataRef = this.cleanDateField(mappedRow.mapped.data_ref, 'data_ref', warnings);
        if (cleanedDataRef !== null) {
            cleaned.data_ref = cleanedDataRef;
        }
        // Clean turno
        const cleanedTurno = this.cleanTextField(mappedRow.mapped.turno, 'turno', warnings, { toUpperCase: true, maxLength: 50 });
        cleaned.turno = cleanedTurno;
        // Clean equipamento (critical field)
        const cleanedEquipamento = this.cleanEquipamentoField(mappedRow.mapped.equipamento, warnings);
        if (cleanedEquipamento !== null) {
            cleaned.equipamento = cleanedEquipamento;
        }
        // Clean curral_codigo
        const cleanedCurralCodigo = this.cleanTextField(mappedRow.mapped.curral_codigo, 'curral_codigo', warnings, { toUpperCase: true, maxLength: 100 });
        if (cleanedCurralCodigo !== null) {
            cleaned.curral_codigo = cleanedCurralCodigo;
        }
        // Clean dieta_nome
        const cleanedDietaNome = this.cleanTextField(mappedRow.mapped.dieta_nome, 'dieta_nome', warnings, { maxLength: 200 });
        cleaned.dieta_nome = cleanedDietaNome;
        // Clean numeric fields
        const cleanedKgPlanejado = this.cleanNumericField(mappedRow.mapped.kg_planejado, 'kg_planejado', warnings);
        if (cleanedKgPlanejado !== null) {
            cleaned.kg_planejado = cleanedKgPlanejado;
        }
        const cleanedKgReal = this.cleanNumericField(mappedRow.mapped.kg_real, 'kg_real', warnings);
        if (cleanedKgReal !== null) {
            cleaned.kg_real = cleanedKgReal;
        }
        const cleanedDesvioKg = this.cleanNumericField(mappedRow.mapped.desvio_kg, 'desvio_kg', warnings, { allowNegative: true });
        cleaned.desvio_kg = cleanedDesvioKg;
        const cleanedDesvioPct = this.cleanNumericField(mappedRow.mapped.desvio_pct, 'desvio_pct', warnings, { allowNegative: true });
     ...
     */
    cleanDateField(value, fieldName, warnings) {
     ...
     */
    cleanTextField(value, fieldName, warnings, options = {}) {
        if (value === null || value === undefined || value === '') {
            return null;
        }
     ...
     */
    cleanEquipamentoField(value, warnings) {
        if (value === null || value === undefined || value === '') {
            return null;
        }
     ...
     */
    cleanNumericField(value, fieldName, warnings, options = {}) {
        if (value === null || value === undefined || value === '') {
            return null;
        }
        // If already a number
...
//# sourceMappingURL=data-cleanser.js.map