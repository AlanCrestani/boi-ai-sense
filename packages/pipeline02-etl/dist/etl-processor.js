...
const parseCSV = async (_content, _options) => ({
    rows: [],
...
export class Pipeline02ETLProcessor {
    ...
    constructor(config) {
        ...
        this.headerMapper = {
            analyzeHeaders: (_headers) => ({ confidence: 0.8, missingRequired: [], unmappedHeaders: [] }),
            mapRow: (raw, _headers, rowNumber) => ({
                raw,
                rowNumber,
                mapped: {},
                errors: []
     ...
     */
    async processCSVFile(csvContent) {
        ...
        try {
            ...
            if (headerAnalysis.confidence < 0.7) {
                await this.logWarning('HEADER_CONFIDENCE_LOW', `Confiança baixa no mapeamento de headers: ${headerAnalysis.confidence}`, {
            ...
            for (let i = 0; i < rows.length; i += batchSize) {
                ...
                await this.logInfo('BATCH_PROCESSED', `Batch processado: ${i + 1}-${Math.min(i + batchSize, rows.length)}`, {
                    validRows: batchResult.validRows,
                    invalidRows: batchResult.invalidRows,
                });
            }
     ...
     */
    async processBatch(rows, headers) {
        let validRows = 0;
        let invalidRows = 0;
        let stagingInserts = 0;
        let factTableUpserts = 0;
        ...
        return await this.db.transaction(async (tx) => {
            for (const row of rows) {
                try {
                    ...
                    const processedData = this.validator.processData(validationResult.data, this.config.organizationId);
                    ...
                    const upsertResult = await this.upsertService.upsertSingleRecord(tx, processedData, this.config.organizationId, this.config.fileId, fatoDesvioCarregamento);
                    factTableUpserts++;
     ...
     */
    async logInfo(step, message, context) {
     ...
     * Log warning message
     */
    async logWarning(step, message, context) {
        if (this.config.runId) {
            await this.db.insert(etlRunLog).values({
                runId: this.config.runId,
                organizationId: this.config.organizationId,
     ...
     * Log error message
     */
    async logError(step, message, context) {
        if (this.config.runId) {
            await this.db.insert(etlRunLog).values({
                runId: this.config.runId,
                organizationId: this.config.organizationId,
