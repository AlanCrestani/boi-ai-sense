 ...
 */
export const VALID_EQUIPAMENTOS = ['BAHMAN', 'SILOKING'] as const;
...
export const pipeline02RawDataSchema = z.object({
  ...
  equipamento: z
    ...
    .refine(
      (val) => VALID_EQUIPAMENTOS.includes(val as any),
      `Equipamento deve ser um dos seguintes: ${VALID_EQUIPAMENTOS.join(', ')}`
    ),
...
export class Pipeline02BusinessValidator {
  ...
  validateRawData(rawData: Record<string, any>): ValidationResult<Pipeline02RawData> {
    try {
      ...
      return {
        isValid: true,
        data: validatedData as Pipeline02RawData,
        errors: [],
      };
    } catch (error) {
      if (error instanceof z.ZodError) {
        const validationErrors: ValidationError[] = error.errors.map((err) => ({
          field: err.path.join('.'),
          message: err.message,
          value: rawData[err.path[0] as string],
          code: err.code,
        }));
        return {
          isValid: false,
  ...
  processData(rawData: Pipeline02RawData, organizationId: string): Pipeline02ProcessedData {
    ...
    return {
      ...
      equipamento: rawData.equipamento as typeof VALID_EQUIPAMENTOS[number],
   ...
   */
  private generateNaturalKey(data: Pipeline02RawData, organizationId: string): string {
    const parts = [
