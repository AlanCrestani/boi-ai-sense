 ...
 */
function analyzeSeparators(lines, separators) {
    const analysis = {};
 ...
 */
function selectBestSeparator(analysis) {
    const separators = [',', ';', '\t', '|'];
    let bestSeparator = ',';
    let bestScore = 0;
    for (const sep of separators) {
 ...
 */
function countOccurrences(line, separator) {
    let count = 0;
    let inQuotes = false;
    let quoteChar = '';
 ...
 */
export function separatorToName(separator) {
    switch (separator) {
        case ',': return 'comma';
