...
export class ETLStateMachine {
  ...
  async transitionState(
    ...
    try {
      ...
      if (fileId) {
        const fileUpdate = this.updateFileState(
          supabaseClient,
          fileId,
          toState,
          stateMetadata
      ...
      if (runId) {
        const runUpdate = this.updateRunState(
          supabaseClient,
          runId,
          toState,
          stateMetadata
      ...
      // Log the transition
      const logEntry = this.logTransition(
        supabaseClient,
        {
          fileId: fileId || '',
          runId: runId || '',
  ...
  private async updateRunState(
    ...
    if (fetchError) throw new Error(`Failed to fetch run: ${fetchError.message}`);
   ...
   */
  async checkStaleLocks(supabaseClient: any): Promise<string[]> {
    ...
    return data?.map((run: any) => run.id) || [];
  }

  /**
   * Release stale lock and transition to failed state
   */
  async releaseStaLock(
    supabaseClient: any,
    runId: string,
