...
export class OptimisticLockingService {
  ...
  private defaultOptions: Required<LockingOptions> = {
    maxRetries: 3,
    retryDelayMs: 100,
    backoffMultiplier: 2,
    sessionId: '',
   ...
   */
  async updateWithLock<T extends VersionedRecord>(
    tableName: string,
    id: string,
    updates: Partial<T>,
   ...
   */
  async updateMultipleWithLock<T extends VersionedRecord>(
    updates: Array<{
      tableName: string;
      id: string;
      updates: Partial<T>;
   ...
   */
  async getRecordVersion(tableName: string, id: string): Promise<{
    exists: boolean;
    version?: number;
    data?: any;
  }> {
  ...
  async lockRecord(
    ...
    lockId?: string
  ): Promise<LockingResult<{ lockId: string; expiresAt: Date }>> {
   ...
   */
  async releaseExpiredLocks(tableName: string): Promise<{
   ...
   */
  async getLockingStats(tableName: string): Promise<{
    totalRecords: number;
   ...
   */
  async safeStateTransition(
    request: ETLStateTransitionRequest,
    options?: LockingOptions
  ): Promise<LockingResult<any>> {
    const opts = { ...this.defaultOptions, ...options };
    let lastError: string = '';
    ...
    try {
      ...
      for (let attempt = 0; attempt <= opts.maxRetries; attempt++) {
        try {
          ...
          (updateData as any).state_history = stateHistory;
   ...
   */
  async clearStaleProcessingSessions(
    tableName: string,
    timeoutMs: number = this.defaultOptions.processingTimeout
  ): Promise<{
    clearedCount: number;
    errors: string[];
  }> {
   ...
   */
  private isProcessingSessionActive(record: any): boolean {
   ...
   */
  async releaseProcessingSession(
    tableName: string,
    recordId: string,
    sessionId: string
  ): Promise<LockingResult<void>> {
