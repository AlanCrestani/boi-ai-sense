...
export class ChecksumService {
  ...
  private defaultAlgorithm: 'sha256' | 'md5' = 'sha256';

  constructor(supabaseClient: any, algorithm: 'sha256' | 'md5' = 'sha256') {
  ...
  async calculateFileChecksum(
    filepath: string,
    algorithm: 'sha256' | 'md5' = this.defaultAlgorithm
  ): Promise<string> {
    try {
   ...
   */
  private shouldAllowReprocessing(originalFile: any): boolean {
   ...
   */
  async updateFileChecksum(
    fileId: string,
    newChecksum: string,
    userId: string,
    reason: string = 'File content updated'
  ): Promise<void> {
    try {
   ...
   */
  private async logReprocessingEvent(event: {
   ...
   */
  async cleanupOldReprocessingRecords(
    organizationId: string,
