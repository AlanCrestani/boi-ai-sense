...
export async function testUpsertLogic(): Promise<void> {
  ...
  const newRecord: Pipeline04ProcessedData = {
  ...
  const insertResult = await upsertService.upsertSingleRecord(
    mockTx,
    newRecord,
    'test-org-123',
    'file-123',
  ...
  const updatedRecord: Pipeline04ProcessedData = {
    ...newRecord,
    quantidade_kg: 175.0,
  ...
  const updateResult = await upsertService.upsertSingleRecord(
    mockTx,
    updatedRecord,
    'test-org-123',
    'file-123',
  ...
  const skipResult = await upsertService.upsertSingleRecord(
    mockTx,
    updatedRecord, // Same record with same data
    'test-org-123',
    'file-123',
    mockFactTable
  );
  ...
  const unknownCurralRecord: Pipeline04ProcessedData = {
    ...newRecord,
    curral_codigo: 'CUR-999', // Non-existing
    natural_key: 'TEST-ORG_2024-12-15_08:30_CUR-999_JOAO_SILVA_RAÇÃO',
  };

  const pendingResult = await upsertService.upsertSingleRecord(
    mockTx,
    unknownCurralRecord,
    'test-org-123',
    'file-123',
  ...
  const unknownDietaRecord: Pipeline04ProcessedData = {
    ...newRecord,
  ...
  const pendingDietaResult = await upsertService.upsertSingleRecord(
    mockTx,
    unknownDietaRecord,
    'test-org-123',
    'file-123',
  ...
  const nullDietaRecord: Pipeline04ProcessedData = {
    ...newRecord,
    dieta_nome: null,
    natural_key: 'TEST-ORG_2024-12-15_08:30_CUR-001_JOAO_SILVA_RAÇÃO_NULL',
  };

  const nullDietaResult = await upsertService.upsertSingleRecord(
    mockTx,
    nullDietaRecord,
    'test-org-123',
    'file-123',
  ...
  const batchData: Pipeline04ProcessedData[] = [
    {
      ...newRecord,
  ...
  const batchResult = await upsertService.upsertBatch(
    mockTx,
    batchData,
    'test-org-123',
    'file-123',
...
class MockTransaction {
  ...
  async findRecord(organizationId: string, naturalKey: string): Promise<any> {
    const key = `${organizationId}:${naturalKey}`;
    return this.records.get(key) || null;
  }

  async updateRecord(organizationId: string, naturalKey: string, data: any): Promise<void> {
    const key = `${organizationId}:${naturalKey}`;
  ...
  async insertRecord(data: any): Promise<string> {
  ...
  async execute(_query: string, params: any[]) {
 ...
 */
class MockFactTable {
  // This would represent the actual table schema in a real implementation
}