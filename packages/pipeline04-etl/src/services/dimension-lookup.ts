...
export class MockDimensionLookupService implements DimensionLookupService {
  ...
  private trateiroCache = new Map<string, string>();
  private pendingEntries = new Map<string, PendingEntry>();

  async lookupCurralId(curralCodigo: string, organizationId: string): Promise<string | null> {
  ...
  async lookupTrateiroId(trateiro: string, organizationId: string): Promise<string> {
  ...
  async createPendingCurral(curralCodigo: string, organizationId: string): Promise<string> {
  ...
  async createPendingDieta(dietaNome: string, organizationId: string): Promise<string> {
   ...
   */
  async getPendingEntries(organizationId: string): Promise<PendingEntry[]> {
