...
import { Pipeline04ProcessingResult } from './pipeline04-processor.js';
...
export class Pipeline04UIService {
   ...
   */
  async getUIPendingEntries(): Promise<UIPendingEntry[]> {
    try {
      ...
      const enhancedEntries: UIPendingEntry[] = await Promise.all(
        pendingEntries.map(async (entry: any) => {
  ...
  async resolvePendingEntry(
    ...
    try {
      ...
      const { error: stagingError } = await this.supabaseClient
        ...
        .eq('organization_id', this.organizationId)
        .eq(
          pendingEntry.type === 'curral' ? 'curral_codigo' : 'dieta_nome',
          pendingEntry.code
        )
        .eq('status', 'pending_resolution');
   ...
   */
  async getProcessingStats(): Promise<UIProcessingStats> {
    try {
      // Get basic stats from multiple tables
      const [
        totalProcessedResult,
        todayProcessedResult,
        weekProcessedResult,
        pendingEntriesResult,
        recentProcessingResult,
      ] = await Promise.all([
      ...
      const pendingCurrals = pendingEntriesResult.data?.filter(
        (e: any) => e.type === 'curral'
      ).length || 0;
      const pendingDietas = pendingEntriesResult.data?.filter(
        (e: any) => e.type === 'dieta'
      ).length || 0;
      ...
      let processingHealthStatus: 'healthy' | 'warning' | 'error' = 'healthy';
   ...
   */
  async getProcessingLogs(limit: number = 50): Promise<ProcessingLogEntry[]> {
    try {
   ...
   */
  async exportProcessingData(dateRange: { start: Date; end: Date }): Promise<string> {
    try {
