...
import { DimensionIds } from './upsert-service.js';
...
export class DimensionLookupService {
  private context: DimensionLookupContext;
  ...
  async lookupDimensions(
    ...
    const pendingCreations: { curral?: string; dieta?: string; equipamento?: string } = {};
   ...
   */
  async lookupCurral(codigo: string): Promise<string> {
    try {
   ...
   */
  async lookupDieta(nome: string): Promise<string | null> {
    try {
   ...
   */
  async lookupEquipamento(equipamento: string): Promise<string | null> {
    try {
   ...
   * Create pending curral entry
   */
  private async createPendingCurral(codigo: string): Promise<string> {
   ...
   * Create pending dieta entry
   */
  private async createPendingDieta(nome: string): Promise<string> {
   ...
   * Create pending equipamento entry
   */
  private async createPendingEquipamento(equipamento: string): Promise<string> {
   ...
   */
  async getPendingDimensions(): Promise<{
  ...
  async getDimensionStats(): Promise<{
    currais: { total: number; active: number; pending: number };
    dietas: { total: number; active: number; pending: number };
    equipamentos: { total: number; active: number; pending: number };
  }> {
    try {
   ...
   */
  private async getDimensionStatsForTable(
    tableName: string,
    idField: string
    ...
    try {
      ...
      const pending = data?.filter((d: any) => !d.is_active && d[idField].startsWith('pending-')).length || 0;
  ...
  async batchLookupDimensions(
    ...
    for (const record of records) {
      const result = await this.lookupDimensions(
        record.curralCodigo,
        record.equipamento,
        record.dietaNome
      );
