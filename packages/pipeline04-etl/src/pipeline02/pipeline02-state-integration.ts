...
import { ETLState, StateTransitionRequest, ETLFile, ETLRun } from '../state-machine/types.js';
import { DataValidationService, ValidationResult } from './data-validation-service.js';
import { DimensionLookupService, DimensionLookupResult } from './dimension-lookup-service.js';
import { UpsertService, BatchUpsertResult } from './upsert-service.js';
...
export class Pipeline02StateIntegration {
  ...
  async processFile(
    rawData: any[],
    context: Pipeline02Context,
    options: Pipeline02ProcessingOptions = {}
  ): Promise<Pipeline02ProcessingResult> {
    const {
    ...
    const result: Pipeline02ProcessingResult = {
      success: false,
    ...
    try {
      ...
      if (loadingResult.success) {
        await this.transitionState(context, ETLState.LOADING, ETLState.LOADED,
          `Loading complete: ${result.processedRecords} processed, ${result.failedRecords} failed`);
        result.state = ETLState.LOADED;
        result.success = true;
      } else {
        result.errors.push(...loadingResult.errors);
        await this.transitionState(context, ETLState.LOADING, ETLState.FAILED,
          `Loading failed: ${loadingResult.errors.join(', ')}`);
        result.state = ETLState.FAILED;
      }
   ...
   */
  private async validateData(rawData: any[], context: Pipeline02Context): Promise<{
    hasErrors: boolean;
    validRecords: any[];
    errors: Array<{ message: string; record?: any }>;
    warnings: Array<{ message: string; record?: any }>;
  }> {
    const errors: Array<{ message: string; record?: any }> = [];
    const warnings: Array<{ message: string; record?: any }> = [];
   ...
   */
  private async loadData(rawData: any[], options: {
    maxRecordsPerBatch: number;
    autoApproveDimensions: boolean;
    retryFailedRecords: boolean;
  }): Promise<{
    ...
    try {
      ...
      for (const record of rawData) {
        try {
          ...
          // Lookup dimensions
          const dimensionResult = await this.dimensionLookupService.lookupDimensions(
            record.curralCodigo || record.curral,
            record.equipamento,
            record.dietaNome || record.dieta
          );

          // Track dimension statistics
          if (dimensionResult.pendingCreations.curral) pendingDimensions++;
          if (dimensionResult.pendingCreations.dieta) pendingDimensions++;
          if (dimensionResult.pendingCreations.equipamento) pendingDimensions++;

          if (dimensionResult.curralId && !dimensionResult.curralId.startsWith('pending-')) resolvedDimensions++;
          if (dimensionResult.dietaId && !dimensionResult.dietaId.startsWith('pending-')) resolvedDimensions++;
          if (dimensionResult.equipamentoId && !dimensionResult.equipamentoId.startsWith('pending-')) resolvedDimensions++;
  ...
  private async transitionState(
    ...
    const result = await this.stateMachineService.transitionFileState(
      context.fileId,
      toState,
      context.userId,
      message
   ...
   */
  async getCurrentState(fileId: string): Promise<ETLState | null> {
    try {
