 ...
 */
export const VALID_TURNOS = ['MANHÃ', 'TARDE', 'NOITE'];
/**
 * Valid treatment types
 */
export const VALID_TIPOS_TRATO = ['RAÇÃO', 'VOLUMOSO', 'MINERAL', 'MEDICAMENTO'];
/**
...
export const pipeline04RawDataSchema = z.object({
    ...
    turno: z.string()
        ...
        .refine((val) => VALID_TURNOS.includes(val), `Turno deve ser um dos seguintes: ${VALID_TURNOS.join(', ')}`),
    ...
    tipo_trato: z.string()
        ...
        .refine((val) => VALID_TIPOS_TRATO.includes(val), `Tipo de trato deve ser um dos seguintes: ${VALID_TIPOS_TRATO.join(', ')}`),
    ...
    quantidade_cabecas: z.number()
        .optional()
        .nullable()
        .refine((val) => val === null || val === undefined || val > 0, 'Quantidade de cabeças deve ser positiva'),
...
export class Pipeline04BusinessValidator {
     ...
     */
    processData(data, organizationId) {
        // Parse date and time
        ...
        const naturalKey = this.generateNaturalKey(organizationId, dataRef, data.hora, data.curral_codigo, data.trateiro, data.tipo_trato);
        return {
            data_ref: dataRef,
            hora: data.hora,
     ...
     */
    generateNaturalKey(organizationId, dataRef, hora, curralCodigo, trateiro, tipoTrato) {
