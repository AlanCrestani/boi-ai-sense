...
const createMockSupabaseClient = () => {
    ...
    return {
        from: (tableName) => ({
            select: (fields = '*', options) => {
                if (fields.includes('count') || options?.count === 'exact') {
                    return {
                        eq: (field, value) => ({
                            eq: (field2, value2) => ({ count: Object.keys(records).length, error: null }),
                ...
                return {
                    eq: (field, value) => ({
                        single: async () => {
                            const record = Object.values(records).find((r) => r[field] === value);
                            return record ? { data: record, error: null } : { data: null, error: { message: 'Record not found' } };
                        },
                        then: async (resolve) => resolve({ data: Object.values(records), error: null }),
                    }),
                    then: async (resolve) => resolve({ data: Object.values(records), error: null }),
                };
            },
            update: (updateData) => ({
                eq: (field, value) => ({
                    eq: (field2, value2) => ({
                        select: () => ({
                            single: async () => {
                                const record = Object.values(records).find((r) => r[field] === value && r[field2] === value2);
                                if (record) {
                    ...
                    then: async (resolve) => {
                        const matchingRecords = Object.values(records).filter((r) => r[field] === value);
                ...
                lt: (field, value) => ({
                    not: (field2, operator, value2) => ({
                        select: async () => {
                            const matchingRecords = Object.values(records).filter((r) => {
        ...
        mockGetRecord: (id) => records[id],
        mockSetRecord: (id, data) => {
            records[id] = data;
        },
...
async function runOptimisticLockingTests() {
    ...
    try {
        ...
        const updateResult = await lockingService.updateWithLock('etl_file', 'test-file-001', { metadata: { test: 'update' } }, { maxRetries: 1 });
        ...
        const conflictResult = await lockingService.updateWithLock('etl_file', 'test-file-001', { metadata: { test: 'conflict' } }, { maxRetries: 1 });
        ...
        const releaseResult = await lockingService.releaseProcessingSession('etl_file', 'test-file-001', 'test-session-001');
        ...
        const multipleUpdates = [
            {
                tableName: 'etl_file',
                id: 'test-file-001',
                updates: { metadata: { bulk: 'update1' } },
                expectedVersion: 1,
            },
            {
                tableName: 'etl_run',
                id: 'test-run-001',
                updates: { metadata: { bulk: 'update2' } },
                expectedVersion: 1,
            },
        ];
        ...
        const incrementResult = await lockingService.incrementCounter('etl_run', 'test-run-001', 'records_processed', 5);
