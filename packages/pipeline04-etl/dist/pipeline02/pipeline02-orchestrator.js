...
import { Pipeline02StateIntegration } from './pipeline02-state-integration.js';
...
export class Pipeline02Orchestrator {
    ...
    async processFile(request) {
        ...
        try {
            ...
            await this.logStep(request.fileId, 'ORCHESTRATION_START', `Starting Pipeline 02 orchestration for file: ${request.fileName}`);
            ...
            if (etlFile.currentState === ETLState.UPLOADED) {
                ...
                try {
                    ...
                    await this.logStep(request.fileId, 'PARSING_SUCCESS', `Successfully parsed ${rawData.length} records from CSV`);
                }
                catch (parseError) {
                    ...
                    await this.stateMachineService.transitionFileState(request.fileId, ETLState.FAILED, this.context.userId, errorMsg);
                ...
                await this.logStep(request.fileId, 'REPROCESSING_NOTE', `File already processed to state: ${etlFile.currentState}. Re-parsing for reprocessing.`);
                try {
                    rawData = await this.parseCSVFile(request.filePath);
                    result.summary.totalRecords = rawData.length;
                }
            ...
            await this.logStep(request.fileId, 'PIPELINE_PROCESSING_START', 'Starting Pipeline 02 state integration processing');
            ...
            try {
                const processingResult = await pipeline02Integration.processFile(rawData, pipelineContext, request.processingOptions || {});
                ...
                if (processingResult.success) {
                    await this.logStep(request.fileId, 'PIPELINE_PROCESSING_SUCCESS', `Pipeline 02 processing completed successfully. Processed: ${processingResult.processedRecords}, Failed: ${processingResult.failedRecords}`);
                }
                else {
                    await this.logStep(request.fileId, 'PIPELINE_PROCESSING_FAILED', `Pipeline 02 processing failed with errors: ${processingResult.errors.join(', ')}`);
                }
            }
     ...
     */
    async logStep(fileId, stepName, message) {
        try {
    ...
    async getFileStatus(fileId) {
        try {
            ...
            const logs = []; // Would get logs from logging service
     ...
     */
    async retryFileProcessing(fileId, filePath, fileName, options) {
        await this.logStep(fileId, 'RETRY_PROCESSING_START', `Starting retry processing for file: ${fileName}`);
        // Reset file state to uploaded for reprocessing
        await this.stateMachineService.transitionFileState(fileId, ETLState.UPLOADED, this.context.userId, 'Retrying processing - resetting to uploaded state');
