 ...
 */
import { UpsertService, DimensionLookupService, } from '../index.js';
            ...
            select: (fields = '*') => {
                const chainable = {
                    eq: (field, value) => {
                        const filteredRecords = tableName === 'fato_desvio_carregamento'
                            ? mockFactRecords.filter((r) => r[field] === value)
                            : (mockDimensions[tableName] || []).filter((r) => r[field] === value);
                        return {
                            eq: (field2, value2) => {
                                const doubleFiltered = filteredRecords.filter((r) => r[field2] === value2);
                                return {
                                    eq: (field3, value3) => {
                                        const tripleFiltered = doubleFiltered.filter((r) => r[field3] === value3);
                                        return {
                                            like: (field4, pattern) => ({
                                                data: tripleFiltered.filter((r) => r[field4] && r[field4].toString().includes(pattern.replace('%', ''))),
                                                error: null,
                                            ...
                                            data: tripleFiltered,
                                            error: null,
                                            then: (resolve) => resolve({ data: tripleFiltered, error: null }),
                                        };
                                    },
                                    like: (field3, pattern) => {
                                        const likeFiltered = doubleFiltered.filter((r) => r[field3] && r[field3].toString().includes(pattern.replace('%', '')));
                                        return {
                                            data: likeFiltered,
                                            error: null,
                                            then: (resolve) => resolve({ data: likeFiltered, error: null }),
                                        };
                                    },
                                    single: async () => {
                            ...
                            in: (field2, values) => ({
                                data: filteredRecords.filter((r) => values.includes(r[field2])),
                                error: null,
                            }),
                    ...
                    in: (field, values) => ({
                        data: mockFactRecords.filter((r) => values.includes(r[field])),
                        error: null,
                    }),
                    or: (condition) => ({
                        eq: (field2, value2) => ({
                            single: async () => {
                                ...
                                const record = records.find((r) => r[field2] === value2);
                                return record ? { data: record, error: null } : { data: null, error: { code: 'PGRST116' } };
                            },
                        }),
                    }),
                    like: (field, pattern) => {
                        ...
                        const filtered = allRecords.filter((r) => r[field] && r[field].toString().includes(pattern.replace('%', '')));
                        return {
                            data: filtered,
                            error: null,
                ...
                if (tableName === 'fato_desvio_carregamento') {
                    Object.assign(chainable, {
                        ...
                        then: (resolve) => resolve({ data: mockFactRecords, error: null }),
                    });
                }
                ...
                eq: (field, value) => ({
                    ...
                    then: async (resolve) => {
                        if (tableName === 'fato_desvio_carregamento') {
                            ...
                            const records = mockDimensions[tableName] || [];
                            const recordIndex = records.findIndex((r) => r[field] === value);
                            if (recordIndex >= 0) {
                                records[recordIndex] = { ...records[recordIndex], ...updates };
                            }
                        }
