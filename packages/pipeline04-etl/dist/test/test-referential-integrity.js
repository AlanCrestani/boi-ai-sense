...
export async function testReferentialIntegrity() {
    ...
    const hasSuspiciousWarning = suspiciousResult.warnings.some(w => w.field === 'curral_codigo' && w.message.includes('suspeito'));
    ...
    const hasLongNameWarning = longNameResult.warnings.some(w => w.field === 'trateiro' && w.message.includes('muito longo'));
    ...
    const hasDuplicateWarning = duplicateResult.warnings.some(w => w.field === 'trateiro' && w.message.includes('duplicata'));
