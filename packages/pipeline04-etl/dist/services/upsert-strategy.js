...
export class FactTratoUpsertService {
    ...
    async upsertSingleRecord(tx, data, organizationId, fileId, fatoTable) {
        try {
            ...
            if (existing) {
                ...
                const needsUpdate = this.recordNeedsUpdate(existing, data, finalCurralId, finalDietaId, trateiroId, fileId);
                if (!needsUpdate) {
                    return { action: 'skipped', reason: 'No changes detected' };
                }
                // Perform update
                await this.updateExistingRecord(tx, fatoTable, existing.id, data, finalCurralId, finalDietaId, trateiroId, fileId);
                return { action: 'updated', recordId: existing.id };
            }
            else {
                // Insert new record
                const recordId = await this.insertNewRecord(tx, fatoTable, organizationId, data, finalCurralId, finalDietaId, trateiroId, fileId);
     ...
     */
    async nativeUpsert(tx, data, organizationId, fileId, curralId, dietaId, trateiroId, _fatoTable) {
        try {
     ...
     */
    recordNeedsUpdate(existing, newData, curralId, dietaId, trateiroId, fileId) {
     ...
     */
    async updateExistingRecord(tx, fatoTable, recordId, data, curralId, dietaId, trateiroId, fileId) {
     ...
     * Insert new record
     */
    async insertNewRecord(tx, fatoTable, organizationId, data, curralId, dietaId, trateiroId, fileId) {
        // For mock testing, use simplified approach
        if (tx.insertRecord) {
