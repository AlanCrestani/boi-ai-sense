...
import { ReferentialIntegrityService } from './referential-integrity.js';
import { FactTratoUpsertService } from './upsert-strategy.js';
/**
...
export class Pipeline04Processor {
     ...
     */
    async processSingleRecord(rawRow, organizationId, fileId, transaction, fatoTable) {
        const errors = [];
        const warnings = [];
        try {
            ...
            const upsertResult = await this.upsertService.upsertSingleRecord(transaction, processedData, organizationId, fileId, fatoTable);
            return {
                success: true,
                result: upsertResult,
                errors,
     ...
     */
    async processBatch(rawRows, organizationId, fileId, transaction, fatoTable) {
     ...
     */
    async getProcessingHealthCheck(organizationId) {
