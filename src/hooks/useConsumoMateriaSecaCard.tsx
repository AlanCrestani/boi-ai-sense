...
interface ConsumoMateriaSecaCard {
  consumoMedio: number;
  totalAnimais: number;
...
export const useConsumoMateriaSecaCard = () => {
  const { organization } = useAuth();

  return useQuery({
    queryKey: ['consumo-materia-seca-card', organization?.id],
    queryFn: async (): Promise<ConsumoMateriaSecaCard | null> => {
      console.log('useConsumoMateriaSecaCard - Iniciando queryFn');
      ...
      console.log('useConsumoMateriaSecaCard - Usando orgId:', orgId);
      ...
      console.log('useConsumoMateriaSecaCard - Data mais recente:', latestDate);
      console.log('useConsumoMateriaSecaCard - Data anterior:', previousDate);

      // Função para calcular média ponderada de uma data
      const calculateWeightedAverage = async (date: string) => {
        const { data, error } = await supabase
          .from('fato_historico_consumo')
          .select('cms_realizado_kg, qtd_animais')
          .eq('organization_id', orgId)
          .eq('data', date)
          .not('cms_realizado_kg', 'is', null)
          .gt('qtd_animais', 0);
        ...
        data.forEach(record => {
          const consumo = parseFloat(record.cms_realizado_kg) || 0;
      ...
      console.log('useConsumoMateriaSecaCard - Resultado:', {
        consumoMedio: latest.weightedAverage,
