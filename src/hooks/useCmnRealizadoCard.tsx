...
interface CmnRealizadoCard {
  cmnRealizadoMedio: number;
  totalAnimais: number;
  variacaoOntem: number;
  percentualVariacao: string;
}

export const useCmnRealizadoCard = () => {
  const { organization } = useAuth();

  return useQuery({
    queryKey: ['cmn-realizado-card', organization?.id],
    queryFn: async (): Promise<CmnRealizadoCard | null> => {
      console.log('useCmnRealizadoCard - Iniciando queryFn');
      ...
      console.log('useCmnRealizadoCard - Usando orgId:', orgId);
      ...
      const { data: recentDates, error: datesError } = await supabase
        ...
        .not('cmn_realizado_kg', 'is', null)
        .gt('qtd_animais', 0)
        .gt('cmn_realizado_kg', 0)
      ...
      console.log('useCmnRealizadoCard - Data mais recente:', latestDate);
      console.log('useCmnRealizadoCard - Data anterior:', previousDate);

      // Função para calcular média ponderada de uma data
      const calculateWeightedAverage = async (date: string) => {
        const { data, error } = await supabase
          .from('fato_historico_consumo')
          .select('cmn_realizado_kg, qtd_animais')
          .eq('organization_id', orgId)
          .eq('data', date)
          .not('cmn_realizado_kg', 'is', null)
          .gt('qtd_animais', 0)
        ...
        data.forEach(record => {
          const cmn = parseFloat(record.cmn_realizado_kg) || 0;
          const animais = parseInt(record.qtd_animais) || 0;

          totalCmn += cmn * animais;
          totalAnimais += animais;
        });

        const weightedAverage = totalAnimais > 0 ? totalCmn / totalAnimais : 0;
      ...
      console.log('useCmnRealizadoCard - Resultado:', {
        cmnRealizadoMedio: latest.weightedAverage,
