...
serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabaseAdmin.rpc('exec', {
      sql: `
        -- Atualizar a view para ordenar por número do curral, não por lote
        ...
        SELECT
          ...
          dr.lote,
          dr.data,
          EXTRACT(DOW FROM dr.data) as dia_semana,
        ...
        ORDER BY
          -- Ordenar por número do curral primeiro (converte para número quando possível)
          CASE
            WHEN dr.curral ~ '^[0-9]+$' THEN dr.curral::integer
            ELSE 999999
          END,
          -- Depois por curral como texto (para currais não numéricos)
          dr.curral,
          -- Por último por lote
          dr.lote,
          -- E por data
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'View atualizada com ordenação por curral' }),
