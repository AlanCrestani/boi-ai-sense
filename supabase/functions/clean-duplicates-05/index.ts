...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🧹 Iniciando limpeza de duplicados Pipeline 05 para organização: ${organizationId}`);
    ...
    if (idsToDelete.length === 0) {
      return new Response(JSON.stringify({
        success: true,
        message: 'Não há duplicados para remover no Pipeline 05',
    ...
    console.log(`✅ Limpeza Pipeline 05 concluída: ${removedCount} duplicados removidos, ${finalCount} registros restantes`);

    return new Response(JSON.stringify({
      success: true,
      message: `Limpeza de duplicados Pipeline 05 concluída`,
      totalRecords: totalRecords,
    ...
    console.error('❌ Erro na limpeza de duplicados Pipeline 05:', error);
    return new Response(JSON.stringify({
      success: false,
