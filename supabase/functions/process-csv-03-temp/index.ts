...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔄 Processando CSV 03: ${filename} para organização: ${organizationId}`);
    ...
    let filePath = `${organizationId}/03/${filename}`;
    ...
    if (downloadError) {
      filePath = `${organizationId}/csv-processed/03/${filename}`;
    ...
    // Mapear colunas esperadas para staging_03_desvio_distribuicao
    ...
    // Função para normalizar trato (Trato 1 -> 1, Trato 2 -> 2, etc)
    const normalizeTrato = (tratoStr) => {
      if (!tratoStr) return null;
      const str = tratoStr.toString().toLowerCase().trim();
      ...
      // Extrair número de "Trato X"
      const match = str.match(/trato\s*(\d+)/);
      if (match) {
        return match[1];
      }

      // Se não conseguir extrair, retornar original
      return tratoStr;
    };
    ...
    for (let i = 0; i < dataLines.length; i += batchSize) {
      ...
      for (const line of batch) {
        try {
          ...
          const rawTrato = values[columnIndices.trato] || null;
          const trato = normalizeTrato(rawTrato); // Normalizar trato

          // Debug primeiras linhas
          if (i === 0 && batchData.length < 3) {
            console.log(`🔍 Linha ${batchData.length + 1}: rawData=${rawData}, data=${data}, curral=${curral}`);
            console.log(`🔍 Trato normalizado: ${rawTrato} → ${trato}`);
          ...
          const record = {
            ...
            desvio_kg: values[columnIndices.diferenca_kg] ? parseFloat(values[columnIndices.diferenca_kg]) : null,
