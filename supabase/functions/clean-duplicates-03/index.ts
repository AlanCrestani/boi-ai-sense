...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🧹 Iniciando limpeza de duplicados para organização: ${organizationId}`);
    ...
    if (idsToDelete.length === 0) {
      return new Response(JSON.stringify({
        success: true,
        message: 'Não há duplicados para remover',
    ...
    console.log(`✅ Limpeza concluída: ${removedCount} duplicados removidos, ${finalCount} registros restantes`);

    return new Response(JSON.stringify({
      success: true,
      message: `Limpeza de duplicados concluída`,
      totalRecords: totalRecords,
